...
def Atrib(Vars, Exps, wp=True):
    if wp:
        ...
        #        return lambda phi: Exists(freshs, And(Eq(Vars, Exps_), Sub(phi, Vars, freshs)))
        ...
        try:
            ...
            if self.T and self.WP and wp:
                ...
                if self.eq_sat(As, Bs):
                    print("transition relation is WP consistent for %s " % str(this) + "," + str(other))
                    return True
                else:
                    raise Bug("transition relation is WP  inconsistent for %s " % str(this) + "," + str(other))
            elif self.T and self.SP and not wp:
                As = [self.SP(this), other]
                Bs = [this, self.T, Sub(other, self.vars, primes)]
                if not self.eq_sat(As, Bs):
                    raise Bug("transition relation is SP inconsistent for %s " % str(this) + "," + str(other))
                else:
                    print("transition relation is SP consistent for %s " % str(this) + "," + str(other))
                    return True
            else:
...
def main():
    ...
    leftSP = Atrib([y, r, x], [BVLShr(BVSub(y, um), um), BVAdd(r, x), BVLShl(x, um)], wp=False)
