...
def IMC(TS: TransitionSystem,
    ...
    available at https://verify.inf.usi.ch/sites/default/files/RolliniPhDThesis.pdf
...
def lifting_query(s: FNode, z, T, t: FNode):
    """
    The lifting query takes the form :math:`s ∧ z ∧ T ∧ ¬t'`, which asks whether an :math:`s`-state
    has a successor other than :math:`t` under input assignment :math:`z`. Since this is not the case
    by construction, the query yields an unsatisfiable core that typically reveals a
    significantly reduced partial assignment that can replace :math:`s`.

    As per J. Birgmeier, A. R. Bradley, and G. Weissenbacher, “Counterexample to Induction-Guided Abstraction-Refinement (CTIGAR),” p.6.
    """
...
def _lift(k, Inv, Q, s):
    ...
    because :math:`s` is a CTI, and therefore we know that :math:`A ⇒ B` holds.
    Hence, the resulting interpolant satisfies the criteria for :math:`C` to
...
def PDR(P: FNode,
    ...
    """
    Iterative-Deepening k-Induction with Property Direction as specified at D. Beyer and M. Dangl, “Software
    Verification with PDR: Implementation and Empirical Evaluation of the State of the Art” arXiv\:1908.06271 [cs],
    Feb. 2020, Accessed: Mar. 05, 2022. [Online]. Available https://arxiv.org/abs/1908.06271
